import { Controller, Get, Post, Patch, Delete, Param, Body, Query,
         Res, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
...
export class QuotationsController {
  ...
  @ApiOperation({ summary: 'Generate AI quotation' })
  generate(@Body() dto: any, @CurrentUser() u: any) {
    return this.svc.generate(dto, u.tenantId, u.id);
  }

  // ── :id/sub routes MUST come before plain :id ────────────────
  ...
  send(@Param('id') id: string, @Body() dto: any, @CurrentUser() u: any) {
    return this.svc.sendByEmail(id, dto, u.tenantId);
  }
  ...
  update(@Param('id') id: string, @Body() dto: any, @CurrentUser() u: any) {
    return this.svc.update(id, dto, u.tenantId);
  }
  ...
  delete(@Param('id') id: string, @CurrentUser() u: any) {
    return this.svc.delete(id, u.tenantId);
  }
}
